...
Before running the pipelines, ensure the following are configured:
...
4.  **Service Connection**: An Azure DevOps service connection with sufficient permissions (`Contributor` role on the target subscription is typical) to manage resources in your Azure subscription.
...
1.  **Create a Pipeline**: In your Azure DevOps project, create a new pipeline and point it to the main pipeline definition file (e.g., `pipelines/azure-pipelines.yml`) in this repository.

2.  **Define Stages**: In your main pipeline file, define stages for each environment you want to deploy to (e.g., Dev, Prod).

...
#
# Pre-requisites:
#  1. Azure CLI (`az`) must be installed and an active login session is required (`az login`).
#  2. `jq` must be installed for JSON parsing. An active `az login` session is required.
...
#
# This combination allows for building immutable "golden images" with Packer,
...
# Configuration
AZ_AGENT_NAME="csec-devops-agent-$(openssl rand -hex 3)" # Unique agent name
AGENT_VERSION="4.265.1"

# Azure Infrastructure Configuration
RESOURCE_GROUP="csec-devops-rg"
LOCATION="southeastasia"
VNET_NAME="csec-devops-vnet"
SUBNET_NAME="csec-devops-snet"
...
EXISTING_VM=$(az vm list --resource-group "${RESOURCE_GROUP}" --query "[?starts_with(name, 'csec-devops-agent-')].name" -o tsv)

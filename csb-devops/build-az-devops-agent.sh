...
#              following steps:
#
#  1.  Performs pre-flight checks for required tools and environment variables.
...
#   - AZ_ORG : Azure DevOps organization URL (e.g., "https://dev.azure.com/org-name").
...
# Configuration
AZ_AGENT_NAME="csb-az-agent-$(openssl rand -hex 3)" # Unique agent name
...
EXISTING_VM=$(az vm list --resource-group "${RESOURCE_GROUP}" --query "[?starts_with(name, 'csb-az-agent-')].name" -o tsv)
...
ssh -o StrictHostKeyChecking=no -i "${SSH_KEY_PATH}" "${ADMIN_USER}@${PUBLIC_IP}" <<'EOF'
  console_log "Checking Azure DevOps Agent Service Status"
  sudo systemctl status "vsts.agent.$(hostname -f).service" --no-pager
EOF

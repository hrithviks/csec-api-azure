...
#  2. The Azure DevOps extension must be installed (`az extension add --name azure-devops`).
...
console_log "Performing pre-flight checks..."
...
console_log "Pre-flight checks passed."
...
# Check if the pool already exists to make the script idempotent
POOL_ID=$(az pipelines pool list --organization "${AZ_ORG}" --query "[?name=='${AZ_POOL}'].id" -o tsv)

if [ -n "$POOL_ID" ]; then
  console_log "Agent pool '${AZ_POOL}' already exists. Skipping creation."
else
  console_log "Creating agent pool: '${AZ_POOL}'..."
  az pipelines pool create \
    --name "${AZ_POOL}" \
    --pool-type automation \
    --organization "${AZ_ORG}"
  console_log "Agent pool '${AZ_POOL}' created successfully."
fi
...
SC_ID=$(az devops service-endpoint list --organization "${AZ_ORG}" --project "" --query "[?name=='${AZ_RM_SERVICE_CONN}'].id" -o tsv)

if [ -n "$SC_ID" ]; then
  console_log "Service connection '${AZ_RM_SERVICE_CONN}' already exists. Skipping creation."
else
  console_log "Creating service connection '${AZ_RM_SERVICE_CONN}'..."

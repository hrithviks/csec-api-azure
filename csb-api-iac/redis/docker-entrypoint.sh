...
if [ -z "$REDIS_ADMIN_PASSWORD" ] || [ -z "$REDIS_CSB_API_USER" ] || [ -z "$REDIS_CSB_API_PASSWORD" ]; then
...
sed -e "s/\${admin_password}/${SAFE_ADMIN_PASSWORD}/g" \
    -e "s/\${username}/${REDIS_CSB_API_USER}/g" \
    -e "s/\${password}/${SAFE_CSB_API_PASSWORD}/g" \
